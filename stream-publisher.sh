...
gstreamer-publisher --url "$LIVEKIT_URL" --token "$LIVEKIT_PUBLISH_TOKEN" -- \
  "v4l2src device=/dev/video0 ! image/jpeg,framerate=30/1,width=1280,height=720 ! jpegdec ! videoconvert ! $ENCODE_TYPE tune=zerolatency key-int-max=60 bitrate=2000" \
